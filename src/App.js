...
const COLORS = {
  // 메인 (Neutral)
  accent:  '#0E2750',  // 다크 네이비
  accent2: '#C9A86A',  // 인디고
  sub:     '#6E7A8F',  // 그레이
  border:  '#6E7A8F33',  // 라이트 그레이
  bg:      '#ffffff',
  bgAlt:   '#F2F1EC',
  // 상태 (foreground + background 페어)
...
const MotivationWorkbook = () => {
  ...
  const savePartial = () => {
    ...
    const h = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>지원동기 임시저장</title><style>body{font-family:'맑은 고딕',sans-serif;line-height:1.7;padding:40px;white-space:pre-wrap}</style></head><body>${raw}</body></html>`;
    const b = new Blob([h], { type: 'application/msword;charset=utf-8' }); const u = URL.createObjectURL(b);
    const a = document.createElement('a'); a.href = u; a.download = `${basicInfo.company || '회사'}_지원동기_임시저장_${today}.doc`; a.click();
      ...
      <div style={S.container}>
        ...
        <div style={S.cardLarge}>
          ...
          <div style={{ textAlign: 'center', marginBottom: SPACING.xl, paddingTop: SPACING.md }}>
            {/* A 락업 (심볼+워드마크) — 가이드 PART 1-4-6 인터랙티브 첫 화면 */}
            <div style={{ marginBottom: SPACING.base, display: 'flex', justifyContent: 'center' }}>
              <CELockupA height={56} />
            </div>
            {/* ② 슬로건 — 중간 크기, Bold, accent — 락업 아래 */}
          ...
          {/* ═══ 워크북 제목 ═══ */}
          <p style={S.brandEyebrow}>STEP 4 · 지원동기 작성</p>
          <h1 style={S.h1Center}>지원동기</h1>
          <p style={{ ...S.subtitle, textAlign: 'center', marginBottom: SPACING.xl }}>3라운드 체계적 작성으로 완성하는 지원동기</p>
          ...
          <div style={{ background: COLORS.bgAlt, borderRadius: RADIUS.base, padding: SPACING.lg, marginBottom: SPACING.xl }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: SPACING.base }}>
              <div style={S.accentLeft(COLORS.accent)}>
                <h3 style={{ fontSize: FONT.size.md, fontWeight: FONT.weight.bold, color: COLORS.accent, margin: 0, marginBottom: 4 }}>1라운드: 기본 지원동기 수립</h3>
                <p style={{ fontSize: FONT.size.sm, color: COLORS.sub, margin: 0 }}>4개 Q 핵심 질문 — 왜 이 직무 · 왜 이 회사 · 무엇을 준비 · 어떻게 기여</p>
              </div>
              <div style={S.accentLeft(COLORS.accent2)}>
                ...
                <p style={{ fontSize: FONT.size.sm, color: COLORS.sub, margin: 0 }}>부족한 Q 선택 → 심화 질문으로 구체화 (1개 이상)</p>
              </div>
          ...
          <div style={S.boxTip}>
            ...
            items={[
              { id: 'job_analysis', hint: '왜 이 회사인가의 답변 출처' },
              { id: 'experience', hint: '왜 나인가의 답변 출처' }
            ]}
          />

          {/* 경고 (WARNING 박스) — 표준 7-7 필수 문구 */}
    ...
    <div style={S.page}>
      ...
      <div style={S.container}>
        ...
        <div style={S.cardLarge}>
              ...
              <div style={{ background: COLORS.bg, borderLeft: `3px solid ${COLORS.accent2}`, borderRadius: `0 ${RADIUS.sm}px ${RADIUS.sm}px 0`, padding: SPACING.base, marginBottom: SPACING.sm }}>
                ...
                {answers.q1_1 && (
                  <div style={{ background: COLORS.bgAlt, borderRadius: RADIUS.sm, padding: SPACING.sm, marginBottom: 4 }}>
                    ...
                    <p style={{ fontSize: FONT.size.xs, color: COLORS.accent, margin: 0, marginTop: 4, lineHeight: FONT.lineHeight.base }}>{answers.q1_1.substring(0,100)}{answers.q1_1.length>100?'...':''}</p>
                  </div>
                )}
              ...
              <div style={{ background: COLORS.bg, borderLeft: `3px solid ${COLORS.accent2}`, borderRadius: `0 ${RADIUS.sm}px ${RADIUS.sm}px 0`, padding: SPACING.base, marginBottom: SPACING.sm }}>
                ...
                {answers.q3_2 && (
                  <div style={{ background: COLORS.bgAlt, borderRadius: RADIUS.sm, padding: SPACING.sm, marginBottom: 4 }}>
                    ...
                    <p style={{ fontSize: FONT.size.xs, color: COLORS.accent, margin: 0, marginTop: 4, lineHeight: FONT.lineHeight.base }}>{answers.q3_2.substring(0,100)}{answers.q3_2.length>100?'...':''}</p>
                  </div>
                )}
              ...
              <div style={{ background: COLORS.bg, borderLeft: `3px solid ${COLORS.accent2}`, borderRadius: `0 ${RADIUS.sm}px ${RADIUS.sm}px 0`, padding: SPACING.base }}>
                ...
                {answers.q4_1 && (
                  <div style={{ background: COLORS.bgAlt, borderRadius: RADIUS.sm, padding: SPACING.sm, marginBottom: 4 }}>
                    ...
                    <p style={{ fontSize: FONT.size.xs, color: COLORS.accent, margin: 0, marginTop: 4, lineHeight: FONT.lineHeight.base }}>{answers.q4_1.substring(0,100)}{answers.q4_1.length>100?'...':''}</p>
                  </div>
                )}
          ...
          <RelatedWorkbookList
            items={[
              { id: 'job_analysis', hint: '회사를 선택한 이유의 출처 (왜 이 회사인가)' },
              { id: 'experience', hint: '내 경험에서 동기 끌어내기 (왜 나인가)' },
              { id: 'jobcompetency', hint: '직무역량과 함께 자소서 5대항목 완성' },
              { id: 'careergoal', hint: '입사 후 포부와 톤·방향 일치' },
              { id: 'self_introduction', hint: '1분 자기소개 첫 문장 톤 일치' }
            ]}
          />
          <div style={{ ...S.boxTip, marginBottom: SPACING.md }}>
            <p style={{ ...labelStyle(COLORS.yellow), marginBottom: SPACING.sm }}>멘토링 안내</p>
            <p style={{ fontSize: FONT.size.sm, color: COLORS.accent, margin: 0, lineHeight: FONT.lineHeight.base }}>
              자력으로 Q1~Q4 연결이 막힌다면 1:1 <a href="https://www.latpeed.com/products/fKnUV" target="_blank" rel="noopener noreferrer" style={{ color: COLORS.accent2, fontWeight: FONT.weight.semibold, textDecoration: 'underline' }}>자소서 멘토링</a>으로 답변의 논리 흐름을 점검받으세요
            </p>
          </div>
